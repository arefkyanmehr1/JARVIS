...
public class JarvisDb extends SQLiteOpenHelper{
 ...
 public String recentAll(int limit){Cursor c=getReadableDatabase().query("messages",new String[]{"address","name","body","direction","ts"},null,null,null,null,"ts DESC",String.valueOf(limit));StringBuilder s=new StringBuilder();try{while(c.moveToNext()){s.append(c.getString(1)==null||c.getString(1).isEmpty()?c.getString(0):c.getString(1)).append("\n").append(c.getInt(3)==1?"↗ ":"↙ ").append(c.getString(2)).append("\n\n");}}finally{c.close();}return s.toString();}
